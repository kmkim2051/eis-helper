...
- [ ] 브랜치 보호 설정 — main에 PR 필수 + required check(test, test-mariadb) 설정 (GitHub 저장소 설정, 직접). dev push CI는 양 repo green 확인 완료, test-mariadb는 첫 dev→main PR에서 확인
- [ ] CI 개선 4건 적용 (ci-guideline.md §2) — ci 프로파일 분리(application-ci.yml), MariaDB 버전 고정(compose·CI 동일 마이너), artifact에서 *-plain.jar 제외 + SHA 포함 이름, workflow permissions: contents: read
